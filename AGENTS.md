...
- `src/extension.ts` is the runtime entrypoint. It composes `/auto`, `/automodel`, `onPermissionRequest`, prompt-tracking hooks, and shutdown cleanup while keeping command and policy logic in focused modules.
- `src/commands/auto.ts` and `src/commands/automodel.ts` build the slash command handlers. `src/commands/model-formatting.ts` formats classifier model status text shared by those commands.
- `src/permission-policy.ts` owns permission request routing, built-in read/write and read-only MCP auto-approval, shell command enrichment, shell fast-path mapping, classifier fallback, and permission result mapping.
...
- `src/classifier-models.ts` owns classifier model resolution and GitHub Copilot model-list API calls.
...
- `src/types.ts` defines the Zod schemas for shell permission payloads.
- `tests/extension.test.ts`, `tests/classifier.test.ts`, `tests/classifier-models.test.ts`, `tests/shell-safety.test.ts`, and `tests/config.test.ts` are focused unit tests for runtime composition/permission behavior, nested classifier lifecycle, classifier model behavior, fast-path heuristics, and config persistence behavior.
- `extension.mjs` is generated build output. Make source edits in `src/**/*.ts`, then rebuild with Bun.
...
- `npm test -- tests/extension.test.ts` - narrow check for extension permission behavior.
- `npm test -- tests/classifier.test.ts` - narrow check for nested classifier lifecycle behavior.
- `npm test -- tests/classifier-models.test.ts` - narrow check for classifier model listing behavior.
- `npm test` - run the full Vitest suite.
- `npm run typecheck` - run the TypeScript `tsgo` check.
- `npm run build` - bundle `src/extension.ts` into `extension.mjs` with Bun.
...
- Keep permission request decisions conservative and explicit:
  - approve obvious read-only cases quickly
  - return `{ kind: "no-result" }` when the main session should still prompt
  - use `{ kind: "denied-by-permission-request-hook" }` only for deliberate hard blocks
...
- Permission request changes should add or update focused tests that assert exact returned permission results and denial messages.
- Keep the extension and classifier tests aligned when approval semantics change.
- Keep classifier model-list tests in `tests/classifier-models.test.ts` when model catalog behavior changes.
