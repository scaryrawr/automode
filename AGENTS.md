...
- `tests/extension.test.ts`, `tests/classifier.test.ts`, `tests/classifier-models.test.ts`, `tests/shell-safety.test.ts`, and `tests/config.test.ts` are focused unit tests for runtime composition/pre-tool behavior, nested classifier lifecycle, classifier model behavior, fast-path heuristics, and config persistence behavior.
